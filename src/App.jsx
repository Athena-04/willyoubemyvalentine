...
function App() {
      ...
      if (isPlaying) {
        ...
        if (playPromise !== undefined) {
          playPromise
            ...
            .catch((error) => {
              // Handle errors silently - AbortError and other errors during playback
  ...
  const handleSongSelect = useCallback((index) => { 
    ...
    setCurrentTime(0);
    // When song changes, the src will update, so we don't need to manually load 
    // The onCanPlay event will handle automatic playback if isPlaying was true
  }, []);
  ...
  const handleCanPlay = useCallback(() => {
    // Audio is ready to play, automatically resume if it should be playing
  ...
  const handleAudioError = useCallback((e) => {
    ...
    if (error) {
      ...
      // 4 = MEDIA_ERR_SRC_NOT_SUPPORTED (file not found or not supported)
